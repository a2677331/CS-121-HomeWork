...
int main() {
	...
	inFile >> fname >> lname; // read in first name, last name and grade
	inFile >> grade;

	// keep reading the file until cannot read more
	while (inFile) {
		count_students++; // count the number of students

		// SCENARIO 1: when a student has grades, get the average and the highest grade
		if (grade >= 0) {
			while (grade != -1) {
				count_grades++;
				sum += grade;
		...
		// SCENARIO 2: when a student has no grades (except for the -1), their average is 0
		else if (grade == -1){
